...
interface PanelModelFieldsProps {
  panel: Panel;
  onChange: (updates: Partial<Panel>) => void;
}

export default function PanelModelFields({ panel, onChange }: PanelModelFieldsProps) {
  ...
  const handle3DFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // In a real app, you'd upload to /public/models/ or a CDN
        ...
        <div className="mt-4 p-3 bg-gray-50 rounded-md">
          ...
          <div className="w-full h-32 bg-white border border-gray-200 rounded flex items-center justify-center">
            <p className="text-xs text-gray-400">
              {panel.model2D.startsWith('http') ? 'Loading preview...' : 'File: ' + panel.model2D}
            </p>
          </div>
        </div>
      )}
